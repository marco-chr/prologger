...
def poll_projects():
    ...
    for el in job_list:
        ...
        if el[1] == "Hourly":
            script_handle.write("    schedule.every().day.at(\"08:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"09:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"10:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"11:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"12:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"13:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"14:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"15:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")
            script_handle.write("    schedule.every().day.at(\"16:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")

        elif el[1] == "Daily":
            script_handle.write("    schedule.every().day.at(\"08:00\").do(task" + str(el[0]) + ", configuration=configuration)\r\n")

        elif el[1] == "Weekly":
            script_handle.write("    schedule.every().monday.do(task" + str(el[0]) + ", configuration=configuration)\r\n")
